...
export default function Index({lenguajes = [], imagesServer = [], filmsServer = []}) {
  ...
  const handleClose = () => {
    setUsuariosLenguajes([])
    setVisible(true)
    console.log(lenguajeSeleccionado)
    // setLenguajeSeleccionado(lenguajes[0]); //Si no se quedaría el último lengaje seleccionado
  }
  const getUserLang = () => {
    // setVisible(false)
  ...
  const getUsuariosGit = async () => {
    ...
    try {
      const response =
        await axios.get("https://api.github.com/search/users?q=location:Zaragoza");
      console.log(response.data);
                    ...
                    <select onChange={handleChangeLanguaje}>
                      <div className="card-body">
                        ...
                        <div className="card-actions justify-end">
                          ...
                          <button onClick={getUserLang} className="btn btn-primary">
                            Obtener datos
                          </button>
                        </div>
                      </div>
                      {lenguajes.map((lenguaje, index) => (
                        <option
                          key={index} value={lenguaje}>{lenguaje}</option>
                      ))}
                    </select>
                  )}
